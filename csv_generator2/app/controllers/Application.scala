...
import models.{GenerationSession, DAO, User}
...
object Application extends Controller {
  ...
  def logout = Action {
    implicit request =>
      Redirect(routes.Application.index()).flashing("successKey" -> "user_was_logout").withNewSession
  }
  ...
  def connect(session: Session, userId: String): Session = {
    println("Attempt to login user for session: " + session)
    session.+(LOGGED, userId)
  }
...
private static GenerationSession getGs(String json) {
